...
def nav(*buttons):
    """Build inline keyboard. Each arg: (label, command)"""
    rows = []
    row = []
...
async def cmd_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if cmd == "start":
        cmd = "home"
    if cmd in SCREENS:
        text, keyboard = SCREENS[cmd]
        await update.message.reply_text(text, parse_mode="Markdown",
                                        reply_markup=keyboard,
                                        disable_web_page_preview=True)
    else:
        text, keyboard = SCREENS["home"]
        await update.message.reply_text(text, parse_mode="Markdown",
                                        reply_markup=keyboard,
                                        disable_web_page_preview=True)
...
def main():
    ...
    # Register all command screens
    commands = list(SCREENS.keys()) + ["start", "about"]
    for cmd in commands:
